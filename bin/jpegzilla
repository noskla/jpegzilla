...
import math, sys, ntpath, tkinter, tkinter.ttk, tkinter.filedialog, os, subprocess, threading, hurry.filesize
...
class jpegzilla:
    ...
    def __init__(self):
        ...
        # - Place items
        self.gui_options['quality'].place(x=10, y=60)
        self.gui_options['smoothing'].place(x=10, y=130)
        ...
        self.root.bind('<Shift-E>', lambda x:subprocess.Popen(['xdg-open', 'https://waa.ai/XxXN'], stdout=FNULL, stderr=subprocess.STDOUT))
    ...
    def save_all(self):
        ...
        location = tkinter.filedialog.asksaveasfilename(title='Save compressed files', filetypes=(('7Zip archive file', '*.7z'), ('All files', '*.*')))
        print(location)

        os.system('7z a {} /tmp/jpegzilla/'.format(location))
    ...
    def compress(self):
        ...
        for entry in files_to_compress:
            ...
            if entry_data[1] == 'Completed':
                ...
                c = (command.format(filename=(save_dir + img + extension), targa=('-targa' if extension == '.tga' else '')) + ' ' + entry_data[2])

                subprocess.Popen(c, shell=True, stdout=subprocess.PIPE).wait()
                self.file_queue.item(entry, values=( entry_data[0] + ' -> ' + hurry.filesize.size(os.stat(save_dir + img + extension).st_size), 'Completed', entry_data[2] ))
